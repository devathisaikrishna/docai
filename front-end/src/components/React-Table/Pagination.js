		...
		<nav aria-label="..." className="mt-2">
			<ul className="pagination rounded-flat pagination-success">
				<li className="page-item " onClick={() => previousPage()}>
					<button className="page-link" disabled={!canPreviousPage}>
				...
				{visiblePages.map((page, index, array) => {
					if (array[index - 1] + 2 < page) {
						return (
							<>
								<li key={index} className={"page-item " + (pageIndex === index ? "active" : "")} key={index + 1} onClick={() => gotoPage(page - 1)}>
				...
				<li className="page-item" onClick={() => nextPage()}>
					<button className="page-link" disabled={!canNextPage}>
